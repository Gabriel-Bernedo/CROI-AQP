...
classdef C_G_BDController
  ...
  methods
    ...
    function x = read_Electrodomestico(obj)
      ...
      for i = 1:height(res)
        x(i) = C_G_Electrodomestico(res{i,1}, res{i,2}, res{i,3}, res{i,4}, res{i,5}, res{i,6});
      end
      %METHOD1 Summary of this method goes here
      %   Detailed explanation goes here
    end
    ...
    %metodo delete y update funcionan
    function delete_Electrodomestico(obj, id)
      sqlquery = sprintf("DELETE FROM electrodomesticos WHERE id = %d", id);
      exec(obj.connection, sqlquery);
    end

    function update_Electrodomestico(obj, register)
      % Valida que el objeto tenga un ID para actualizar
      if isempty(register.id)
        error('El objeto debe tener un ID para poder ser actualizado.');
      end
      
      % Prepara la consulta SQL para actualizar
      sqlquery = sprintf("UPDATE electrodomesticos SET nombre = '%s', consumo = %d, horas = %d, dias = %d, tipo = '%s' WHERE id = %d", ...
                         register.nombre, register.potencia, register.horas_dia, register.dia_semana, register.tipo, register.id);
